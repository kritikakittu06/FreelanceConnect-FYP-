        ...
        <section>
            <form method="post" action="{{ route('admin.users.update', $user) }}" class="mt-6 space-y-6" enctype="multipart/form-data">
                ...
                <div>
                    ...
                    <x-text-input id="name" name="name" type="name"  :value="old('name', $user->name)" required autofocus autocomplete="name" />
                    <x-input-error class="mt-2" :messages="$errors->get('name')" />
                </div>

                <div>
                <x-input-label for="email" :value="__('Email')" />
                <x-text-input id="email" disabled class="mt-1 block w-full border-0 cursor-not-allowed" :value="old('email', $user->email)" required autocomplete="username" />
                </div>
